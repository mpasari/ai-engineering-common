...
## Step 5b — Delete CLAUDE.md (it is not used by GitHub Copilot)

**What is CLAUDE.md?**
`CLAUDE.md` is a context file for **Claude Code** and **Cursor** users.
GitHub Copilot does NOT read it -- it only reads `.github/copilot-instructions.md`.

Since Telia champions use GitHub Copilot, `CLAUDE.md` serves no purpose
in your repo. It is generated by `npx aec init` for cross-tool compatibility
but can be safely deleted.

**Delete it now:**

```powershell
# Make sure you are in your repo root first
cd "C:\Projects\[your-repo]"

# Delete CLAUDE.md -- GitHub Copilot does not use this file
...
If the file is not there at all (npx aec init did not generate it for your version)
-- that is fine, skip this step.

**Also replace copilot-instructions.md with the lean version** (this one IS used):
...
Write-Host "copilot-instructions.md lines: $((Get-Content '.github\copilot-instructions.md').Count)"
# Should show: 17
```

**Note:** Future versions of the commons will not generate `CLAUDE.md` for Telia.
It will be added back when Telia teams adopt Claude Code or Cursor.
...
| "Your input exceeds the context window" | copilot-instructions.md is too large (500+ lines) | Run Step 5b -- delete CLAUDE.md (not used by Copilot) and replace copilot-instructions.md with the lean 17-line version before opening Copilot Chat |
